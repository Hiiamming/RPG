...
public class Player extends Hero{
    ...
    // mana
    private int manaRegen = 1; // Mana regeneration per second

    // cooldown
    private boolean skillCooldown = false; // Whether the skill is on cooldown
    private int skillCooldownDuration = 60; // Cooldown duration in frames (1 second at 60 FPS)
    ...
    public void setDefaultValues() {
        ...
        switch (this.getName().toLowerCase()) {
            case "sorceress":
                this.setWorldX(gp.tileSize * 3);
                this.setWorldY(gp.tileSize * 3);
                this.setSpeed(15);
    ...
    public void shootProjectile() {
        ...
        if (this.getMana()>= manaCost && !skillCooldown) {
            // Create a new projectile at the player's position and direction
            Projectile projectile = new Projectile(gp, this.getWorldX(), this.getWorldY(), this.getDirection());
            projectiles.add(projectile);

            // Consume mana
            this.setMana(this.getMana() - manaCost);
            System.out.println("Mana used: " + manaCost + ". Remaining mana: " + this.getMana());
    ...
    public void regenerateMana() {
        if (this.getMana() < this.getMaxMana()) {
            ...
            System.out.println("Mana regenerated: " + manaRegen + ". Current mana: " + this.getMana());
        }
    }
    ...
    public void setAttackMonster(int atk, int worldX, int worldY, int attackRange, int maxExp) {
        this.setAtk(atk);
    ...
    public void attackMonster() {
        ...
        while (iterator.hasNext()) {
            ...
            if (distance <= attackRange) {
                monster.receiveDamage(getAtk());
                // If the monster dies, it will be removed from the list in its die() method
                if (monster.isDead) {
                    gainExp(monster.getMaxExp());  // Gain experience when monster dies
                }
            }
        }
    }
    ...
    public void draw(Graphics2D g2) {
        ...
        if (image != null) {
            ...
            System.out.println("Image is null for direction: " + getDirection());
